...
**Goal:** approximate some function $f^*$ (e.g., a classifier $y = f^*(x)$) by defining a mapping $\hat y = f(x;\theta)$ and learning the parameters $\theta$ that give the best approximation.
...
- If $y$ is computed by applying an operation to $x$, we draw a directed edge $x \to y$.
...
**Backward pass** (right→left, i.e., **backpropagation**): to find $\partial J/\partial v$, we "nudge" $v$ slightly and observe the resulting change in $J$: since $J=3v$, $\partial J/\partial v = 3$. Then, using the **chain rule**, we push this derivative backward:
$$\frac{\partial J}{\partial u} = \frac{\partial J}{\partial v}\cdot\frac{\partial v}{\partial u} = 3\cdot 1 = 3, \qquad
\frac{\partial J}{\partial a} = \frac{\partial J}{\partial v}\cdot\frac{\partial v}{\partial a} = 3\cdot 1 = 3$$
$$\frac{\partial J}{\partial b} = \frac{\partial J}{\partial u}\cdot\frac{\partial u}{\partial b} = 3\cdot c, \qquad
\frac{\partial J}{\partial c} = \frac{\partial J}{\partial u}\cdot\frac{\partial u}{\partial b} = 3\cdot b$$
...
$$z = w^Tx + b, \qquad a = \hat y = \sigma(z), \qquad L(a,y) = -\big[y\log a + (1-y)\log(1-a)\big]$$
...
- $g^{[l]}(\cdot)$: activation function of layer $l$ (may differ per layer, e.g., ReLU in hidden layers, sigmoid/softmax at output)

### 3.2 Per-layer forward equations (single example)
$$\boxed{z^{[l]} = W^{[l]}a^{[l-1]} + b^{[l]}}, \qquad \boxed{a^{[l]} = g^{[l]}(z^{[l]})}, \qquad l = 1,\dots,L$$
Final prediction: $\hat y = a^{[L]}$.

### 3.3 Vectorizing over $m$ training examples
Stack the $m$ examples column-wise: $A^{[0]} = X \in \mathbb{R}^{n_x \times m}$. Then for each layer:
...
If every $g^{[l]}$ were the identity function, then $a^{[L]} = W^{[L]}W^{[L-1]}\cdots W^{[1]}x + (\text{bias terms})$ — a composition of *linear* maps is itself just **one** linear map. Stacking layers would add no representational power at all. Non-linear activations are what let depth actually increase the function class the network can represent (recall the Universal Approximation discussion, Module 1).
...
- **Problem:** saturates for large $|z|$ → gradient $\to 0$ → vanishing gradients in deep nets (see §8).
...
- Odd/anti-symmetric: $\tanh(-z) = -\tanh(z)$; derivative is even: $\tanh'(-z)=\tanh'(z)$.
- Worked values: $\tanh(2.0)\approx0.964$, $\tanh(-1.5)\approx-0.905$, $\tanh(0.8)\approx0.664$, $\tanh(-0.3)\approx-0.291$.
...
- **Problem:** "dying ReLU" — if a unit's $z<0$ persistently, its gradient is permanently 0 and it stops learning (leads to Leaky ReLU / Parametric ReLU variants: $f(z)=\max(\alpha z, z)$ with small $\alpha$, e.g., 0.01, so the unit still receives a small gradient when $z<0$).
...
- For numerical stability, always subtract $\max(z)$ before exponentiating: $\text{softmax}(z)_j = e^{z_j-\max(z)}/\sum_k e^{z_k-\max(z)}$ (shown in the worked example, §7.2).
...
$$L = -\sum_{c=1}^{K} y_c \log(\hat y_c), \qquad \hat y = \text{softmax}(z)$$
Because $y$ is one-hot, this reduces to $L = -\log(\hat y_{\text{true class}})$ — the loss only "cares about" the predicted probability mass assigned to the correct class.
...
- **Forward** (left→right): compute the output $\hat y$ and the loss $L$.
- **Backward** (right→left): compute $\partial L/\partial\theta$ for every parameter $\theta = \{W^{[l]}, b^{[l]}\}$, using the chain rule, so that gradient descent can update them.

### 6.2 General layer-wise backprop equations
Define the **error signal** at layer $l$: $\delta^{[l]} = \dfrac{\partial L}{\partial z^{[l]}}$.

**Output layer** ($l=L$): for the sigmoid+BCE or softmax+CE special case,
$$\delta^{[L]} = a^{[L]} - y$$

**Hidden layers** ($l = L-1, \dots, 1$), propagate the error backward:
$$\delta^{[l]} = \big(W^{[l+1]}\big)^T \delta^{[l+1]} \odot g^{[l]\prime}(z^{[l]})$$
($\odot$ = element-wise product; this is exactly the computation graph's backward pass applied layer by layer.)

**Parameter gradients:**
$$\frac{\partial L}{\partial W^{[l]}} = \delta^{[l]} \big(a^{[l-1]}\big)^T, \qquad \frac{\partial L}{\partial b^{[l]}} = \delta^{[l]}$$

**Intuition:** $\delta^{[l]}$ measures "how much layer $l$'s pre-activation is to blame for the final loss." It is obtained by taking the *next* layer's blame $\delta^{[l+1]}$, mapping it backward through that layer's weights $(W^{[l+1]})^T$ (undoing the forward linear map), then multiplying by how sensitive layer $l$'s own activation function was at that point ($g^{[l]\prime}(z^{[l]})$) — exactly mirroring the chain rule derivation in §2.1.
...
where $\eta$ is the learning rate (see Module 3 — Optimization for more advanced update rules).
...
$$x=\begin{bmatrix}1.0\\0.5\end{bmatrix},\quad
W^{[1]}=\begin{bmatrix}0.5 & 0.2\\-0.3 & 0.8\\0.1 & -0.4\end{bmatrix},\quad
b^{[1]}=\begin{bmatrix}0.1\\-0.2\\0.3\end{bmatrix}$$
$$W^{[2]}=\begin{bmatrix}0.4 & -0.1 & 0.6\\0.2 & 0.7 & -0.3\end{bmatrix},\quad
b^{[2]}=\begin{bmatrix}0.2\\-0.1\end{bmatrix},\quad y=\begin{bmatrix}1\\0\end{bmatrix}$$

### 7.2 Forward Propagation

**Step 1 — Layer 1 pre-activation:**
$$z^{[1]} = W^{[1]}x+b^{[1]} = \begin{bmatrix}0.5(1.0)+0.2(0.5)\\-0.3(1.0)+0.8(0.5)\\0.1(1.0)-0.4(0.5)\end{bmatrix}+\begin{bmatrix}0.1\\-0.2\\0.3\end{bmatrix}
=\begin{bmatrix}0.6\\0.1\\-0.1\end{bmatrix}+\begin{bmatrix}0.1\\-0.2\\0.3\end{bmatrix}=\begin{bmatrix}0.7\\-0.1\\0.2\end{bmatrix}$$

**Step 2 — Apply ReLU:**
$$a^{[1]} = \text{ReLU}(z^{[1]}) = \begin{bmatrix}\max(0,0.7)\\\max(0,-0.1)\\\max(0,0.2)\end{bmatrix} = \begin{bmatrix}0.7\\0.0\\0.2\end{bmatrix}$$
Note: neuron 2 is **"dead"** for this input ($z^{[1]}_2<0 \Rightarrow a^{[1]}_2 = 0$).

**Step 3 — Layer 2 pre-activation:**
$$z^{[2]} = W^{[2]}a^{[1]}+b^{[2]} = \begin{bmatrix}0.4(0.7)-0.1(0)+0.6(0.2)\\0.2(0.7)+0.7(0)-0.3(0.2)\end{bmatrix}+\begin{bmatrix}0.2\\-0.1\end{bmatrix}
= \begin{bmatrix}0.4\\0.08\end{bmatrix}+\begin{bmatrix}0.2\\-0.1\end{bmatrix}=\begin{bmatrix}0.6\\-0.02\end{bmatrix}$$

**Step 4 — Apply Sigmoid:**
$$a^{[2]} = \sigma(z^{[2]}) = \begin{bmatrix}\frac{1}{1+e^{-0.6}}\\\frac{1}{1+e^{0.02}}\end{bmatrix}=\begin{bmatrix}0.646\\0.495\end{bmatrix}=\hat y$$

**Forward pass complete: $\hat y = [0.646,\ 0.495]^T$.**

### 7.3 Backward Propagation

**Step 1 — Loss (BCE):**
$$L=-\tfrac12\big[y_1\log(a_1^{[2]})+(1-y_1)\log(1-a_1^{[2]}) + y_2\log(a_2^{[2]})+(1-y_2)\log(1-a_2^{[2]})\big]$$
$$= -\tfrac12\big[\log(0.646)+\log(0.505)\big] = -\tfrac12\big[-0.437-0.683\big]=0.560$$

**Step 2 — Output layer error** (using the sigmoid+BCE shortcut $\delta^{[2]}=a^{[2]}-y$):
...
$$\frac{\partial L}{\partial W^{[2]}} = \delta^{[2]}(a^{[1]})^T = \begin{bmatrix}-0.354\\0.495\end{bmatrix}\begin{bmatrix}0.7&0.0&0.2\end{bmatrix}
=\begin{bmatrix}-0.248 & 0.000 & -0.071\\0.347 & 0.000 & 0.099\end{bmatrix}$$
...
$$(W^{[2]})^T\delta^{[2]} = \begin{bmatrix}0.4&0.2\\-0.1&0.7\\0.6&-0.3\end{bmatrix}\begin{bmatrix}-0.354\\0.495\end{bmatrix}
= \begin{bmatrix}-0.043\\0.382\\-0.361\end{bmatrix}$$
$$g^{[1]\prime}(z^{[1]}) = \begin{bmatrix}1 & (z_1=0.7>0)\\0 & (z_2=-0.1\le0)\\1 & (z_3=0.2>0)\end{bmatrix}$$
$$\delta^{[1]} = (W^{[2]})^T\delta^{[2]}\odot g^{[1]\prime}(z^{[1]}) = \begin{bmatrix}-0.043\\0.382\\-0.361\end{bmatrix}\odot\begin{bmatrix}1\\0\\1\end{bmatrix}=\begin{bmatrix}-0.043\\0.000\\-0.361\end{bmatrix}$$
...
$$\frac{\partial L}{\partial W^{[1]}} = \delta^{[1]}x^T = \begin{bmatrix}-0.043\\0.000\\-0.361\end{bmatrix}\begin{bmatrix}1.0&0.5\end{bmatrix}
=\begin{bmatrix}-0.043 & -0.022\\0.000 & 0.000\\-0.361 & -0.181\end{bmatrix}, \qquad
\frac{\partial L}{\partial b^{[1]}} = \delta^{[1]} = \begin{bmatrix}-0.043\\0.000\\-0.361\end{bmatrix}$$

These four gradients ($\partial L/\partial W^{[1]}, \partial L/\partial b^{[1]}, \partial L/\partial W^{[2]}, \partial L/\partial b^{[2]}$) are exactly what a gradient-descent optimizer needs to update all parameters (see Module 3).

### 7.4 Bonus worked example: Softmax + Cross-Entropy
Given logits $z = [2.0,\,1.0,\,0.1,\,-1.0]^T$ and one-hot target $y=[0,1,0,0]^T$:

**Stabilized softmax:** subtract $\max(z)=2.0$: $z_{\text{stable}}=[0,-1.0,-1.9,-3.0]$
$$e^{z_{\text{stable}}}=[1.000,\,0.368,\,0.150,\,0.050], \qquad \sum = 1.568$$
$$\text{softmax}(z) = [0.638,\,0.235,\,0.096,\,0.032]\quad(\text{sums to }\approx1)$$

**Cross-entropy loss:** only the true class ($j=2$) contributes:
$$L = -\log(0.235) = 1.447$$

**Gradient (the "beautiful" result):**
$$\frac{\partial L}{\partial z} = \hat y - y = [0.638,\,0.235,\,0.096,\,0.032]-[0,1,0,0] = [0.638,\,-0.765,\,0.096,\,0.032]$$
...
$$L \le \log_2\!\left(\frac{M}{d\cdot c}\right)+1$$

**Worked Example 1 — Small tabular dataset** ($d=20$, $M=800$, $c=3$):
$$L \le \log_2(800/60)+1 = \log_2(13.33)+1\approx 4.7 \Rightarrow L\le4$$
Try $[20,40,3]$: parameters $=20\times40+40+40\times3+3=963$ — exceeds budget $800/5=160$. Iteratively shrink: $[20,20,3]\to483$ (too many) → $[20,10,3]\to243$ (too many) → $[20,6,3]\to147$ ✅. **Final: $[20,6,3]$ with regularization.**

**Worked Example 2 — Medium image dataset** ($d=784$, $M=5000$, $c=10$, MNIST-like):
Naively wide $[784,2352,784,10]$ gives $\approx3.7$M parameters — vastly over budget ($5000/5=1000$). Shrinking down: $[784,128,64,10]\to109{,}000$ → $[784,64,32,10]\to52{,}458$ → $[784,32,10]\to25{,}450$ → $[784,16,10]\to12{,}714$. **Even the smallest options need regularization** (dropout, L2) since parameter counts are still far above the naive $M/5$ heuristic — in practice deep-learning image models rely on architectural priors (CNNs, Module 5) rather than raw MLPs to control effective capacity.

**Worked Example 3 — Large text dataset** ($d=1000$, $M=50000$, $c=5$):
$$L\le\log_2(50000/5000)+1=\log_2(10)+1\approx4.3\Rightarrow L\le4$$
Naive $[1000,5000,2000,1000,5]\to\approx17$M parameters — far too many. Reasonable choice: $[1000,512,256,128,5]\to675{,}000$ parameters, i.e. **13.5 parameters/sample** — a workable ratio. **Recommendation: use with batch normalization and dropout.**
...
- `W2.T @ delta2` implements the backward pass through the linear layer ($(W^{[l+1]})^T\delta^{[l+1]}$ from §6.2).
...
2. Backpropagation is **repeated application of the chain rule**, propagating an "error signal" $\delta^{[l]}$ backward and multiplying by local activation derivatives at each layer.
3. Pairing **sigmoid+BCE** or **softmax+cross-entropy** yields the clean gradient $\hat y - y$ at the output — always prefer these standard pairings.
4. **ReLU** avoids the vanishing-gradient problem of sigmoid/tanh for $z>0$, but can "die" for $z\le0$ — both illustrated numerically in §7 and §8.
...
- **Mistake:** treating the "beautiful" gradient $\hat y - y$ as specific to any activation in isolation. **Correction:** it only holds for the **matched pairs** (sigmoid+BCE or softmax+CE) — using, e.g., MSE with a sigmoid output does **not** simplify this way.
...
2. **For the §7 network**, if instead $W^{[2]}$ used **no bias** ($b^{[2]}=0$), recompute $z^{[2]}$ and $a^{[2]}$.
   *Solution:* $z^{[2]}=[0.4,0.18]$, $a^{[2]}=\sigma(0.4)=0.599,\ \sigma(0.18)=0.545$.
...
4. **Vanishing gradient extension:** repeat the §8.1 calculation for a 6-layer network under the same assumptions ($\|W\|=0.8$, $g'=0.25$). What is $|\delta^{[1]}|$?
   *Solution:* factor $0.2$ per layer, 5 backward steps from layer 6 to layer 1: $|\delta^{[1]}| = 1.0\times0.2^5 = 0.00032$ (0.032% of original) — even more severe vanishing.
5. **Design problem:** for $d=50$ features, $M=2000$ samples, $c=4$ classes, apply the depth heuristic $L\le\log_2(M/(d c))+1$ and propose one candidate architecture with parameter count checked against $M/5$.
   *Solution:* $L\le\log_2(2000/200)+1=\log_2(10)+1\approx4.3\Rightarrow L\le4$. Try $[50,20,4]$: params $=50\times20+20+20\times4+4=1084$ vs budget $2000/5=400$ — too many; try $[50,8,4]\to50\times8+8+8\times4+4=444$ — still slightly over; try $[50,6,4]\to50\times6+6+6\times4+4=334\le400$ ✅.
