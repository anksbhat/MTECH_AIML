...
2. **Model** — the computational machinery (a parameterized function $f_\theta$) that maps input data to predictions. In DL, $f_\theta$ is built from many **chained transformations** (layers).
...
4. **Optimization algorithm** — a procedure (typically a variant of **gradient descent**) that searches for parameters $\theta$ minimizing the loss on the training data.

### Intuition — worked mini-example (wake-word detector)
1. Precisely define inputs (audio snippets) and outputs (yes/no).
2. Choose a flexible **model family** (a set of programs parameterized by $\theta$) — same family can serve "Hey Siri" or "Alexa" style tasks.
3. Collect labeled examples (audio + wake/no-wake label).
4. Run a **learning algorithm** — a meta-program that uses the data to select the parameter values $\theta$ that make the model behave well, i.e., **training**.
...
- $f(\cdot)$: activation function (identity, sigmoid, step, ReLU, ... — see Module 2)
- $\hat{y}$: neuron's output (prediction)
...
$$z = \sum_i w_i x_i + b, \qquad h = \text{step}(z) = \begin{cases}1 & z \ge 0\\ 0 & z < 0\end{cases}$$
...
**Update rule** (for target $t$, output $h$, learning rate $\eta$):

$$\Delta w_i = \eta (t - h)\, x_i, \qquad \Delta w_0 = \eta (t-h)\ \, (\text{bias term, } x_0=1)$$
...
Single input $x_1$, target $t = \text{NOT}(x_1)$, $\eta = 1$, initial $w_1 = w_0 = 0$ (here $w_0$ is the bias):

**Epoch 1:**
| $x_1$ | $t$ | $w_1$ | $w_0$ | $z=w_1x_1+w_0$ | $h=\text{step}(z)$ | correct? | update |
|---|---|---|---|---|---|---|---|
| 0 | 1 | 0 | 0 | 0 | 1 | ✅ | none |
| 1 | 0 | 0 | 0 | 0 | 1 | ❌ | $\Delta w_1 = 1(0-1)(1) = -1$, $\Delta w_0 = 1(0-1) = -1$ → $w_1=-1, w_0=-1$ |

**Epoch 2** ($w_1=-1, w_0=-1$):
| $x_1$ | $t$ | $z$ | $h$ | correct? | update |
|---|---|---|---|---|---|
| 0 | 1 | $-1$ | 0 | ❌ | $\Delta w_1 = 1(1-0)(0)=0$, $\Delta w_0=1(1-0)=1$ → $w_1=-1, w_0=0$ |
| 1 | 0 | $-1+0=-1$ | 0 | ✅ | none |

**Epoch 3** ($w_1=-1, w_0=0$): both examples now classified correctly → **converged**. Final: $w_1=-1,\, w_0=0$, i.e. $h = \text{step}(-x_1)$, which correctly implements NOT.
...
- AND: $w_1=1, w_2=1, b=-1.5$ → $z = x_1+x_2-1.5$; only $(1,1)$ gives $z>0$.
- OR: $w_1=1, w_2=1, b=-0.5$ → $z = x_1+x_2-0.5$; any input with at least one 1 gives $z>0$.

The decision boundary is the line $w_1x_1 + w_2x_2 + b = 0$ (equivalently $ax_1+bx_2+d=0$, slope $m=-a/b$, intercept $c=-d/b$).
...
**Proof of non-linear-separability:** Suppose a line $w_1x_1+w_2x_2+b=0$ could separate class-1 points $\{(0,1),(1,0)\}$ from class-0 points $\{(0,0),(1,1)\}$. Then we need:
$$b<0 \;(\text{from }(0,0)),\quad w_1+b\ge 0 \;(\text{from }(1,0)),\quad w_2+b\ge 0\;(\text{from }(0,1)),\quad w_1+w_2+b<0\;(\text{from }(1,1))$$
Adding the middle two inequalities: $w_1+w_2+2b \ge 0 \Rightarrow w_1+w_2+b \ge -b > 0$ (since $b<0$). This **contradicts** the fourth inequality $w_1+w_2+b<0$. Hence **no** linear separator exists — XOR is **not linearly separable**.
...
This is why the hidden layer is called "hidden" — its outputs $h_1, h_2, \dots$ are not directly observed as final predictions; they simply feed the next neuron.

**Key conceptual leap:** the network *learns its own intermediate features* ($h_1,h_2,\dots$) instead of a human hand-designing them.
...
> **Theorem (Cybenko 1989 / Hornik 1991, informal statement):** A feedforward network with a single hidden layer containing a **finite** number of neurons, using a (non-polynomial, e.g. sigmoid) activation function, can approximate **any continuous function** on a compact subset of $\mathbb{R}^n$ to **arbitrary accuracy**, given enough hidden units.
...
- **Forward pass:** input → linear transform ($Z_1 = XW_1+b_1$) → nonlinearity ($A_1=\sigma(Z_1)$) → linear transform ($Z_2=A_1W_2+b_2$) → nonlinearity ($A_2=\sigma(Z_2)$).
- **Loss:** Binary Cross-Entropy, appropriate for binary classification with a sigmoid output.
- **Backpropagation:** because BCE loss combined with a sigmoid output has the elegant simplification $\partial L/\partial Z_2 = A_2 - y$ (derived in Module 2), the output-layer gradient is just the prediction error. This error is then **propagated backward** through $W_2^T$ into the hidden layer, and multiplied by the **local derivative** of the hidden activation ($\sigma'(Z_1) = A_1(1-A_1)$) — this is the chain rule in action (fully derived with numbers in `02_Deep_Feedforward_Networks.md`).
- **Why this works for XOR but a single perceptron fails:** $W_1$ maps the 2D XOR input into a 4-dimensional hidden representation in which the classes become linearly separable; $W_2$ then just needs to draw one hyperplane in that new space.
...
Choose $w_1=1, w_2=1, b=-0.5$ (a valid solution — not unique):

| $x_1$ | $x_2$ | $z=x_1+x_2-0.5$ | step($z$) | target |
|---|---|---|---|---|
|0|0|-0.5|0|0|
|0|1|0.5|1|1|
|1|0|0.5|1|1|
...
1. **Compute perceptron parameters for NAND.** *(Solution: NAND = NOT(AND); negate AND's weights and bias: if AND used $w_1=1,w_2=1,b=-1.5$, NAND uses $w_1=-1,w_2=-1,b=1.5$. Check: $(1,1)\to z=-1-1+1.5=-0.5<0\to h=0$ ✓; all others $\to z\ge 0 \to h=1$ ✓.)*
2. **Prove NOR is linearly separable and give a valid weight set.** *(Solution: NOR = NOT(OR). Using OR's $w_1=1,w_2=1,b=-0.5$, NOR's weights are $w_1=-1,w_2=-1,b=0.5$. Verify all 4 rows.)*
...
4. **Given a trained perceptron with $w_1=2, w_2=-1, b=0$, sketch/describe the decision boundary.** *(Solution: boundary is line $2x_1 - x_2 = 0 \Rightarrow x_2 = 2x_1$, a line through the origin with slope 2; points above the line get $h=0$, below get $h=1$, depending on sign convention.)*
